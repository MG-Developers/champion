$(document).ready(() => {
    ...
    function generateCaptcha() {
        ...
        for (let i = 0; i < 6; i++) {
            myCaptcha += chars.charAt(Math.floor(Math.random() * chars.length))
        }
    ...
    function refreshCaptcha() {
        ...
        $("#captcha-value").text(captcha)
    }

    $("#refreshCaptcha").click(function () {
        refreshCaptchaAndTimer()
    })

    function refreshCaptchaAndTimer() {
        refreshCaptcha()
        resetTimer()
    }
                ...
                success: function (data, status, xhr) {
                    ...
                    if (xhr.status == 200) {
                        ...
                        if (data.error != true) {
                            ...
                            $.ajax({
                                url: `${test[0].url}/reservationMaster/delete?loginUsername=${username}`,
                                async: true,
                                type: "delete",
                                headers: {
